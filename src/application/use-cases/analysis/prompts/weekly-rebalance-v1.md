...
3. **Portfolio Assessment** — interpretive commentary only: what the allocation picture means and position-level notes (this is where HOLD-style notes live). The bucket/asset-class weights, drift vs. target, and concentration-cap measurements are computed in code and rendered as tables outside this narrative — do NOT recompute or restate them as prose or tables here; refer to them by interpretation (e.g. "the US bucket is over-weight"). Reference specific positions with their quantities/PPC for grounding where it aids the commentary.
4. **Week-over-week Comparison** — required when `{{previousAnalysis}}` is not `"none — first run"`. Focus on the *analytical* read of what changed and whether prior suggested orders appear to have been executed. The raw per-position quantity deltas are precomputed and shown separately, so do not re-tabulate them; comment on their significance instead. Do NOT silently repeat a prior unaddressed suggestion with the same rationale.
