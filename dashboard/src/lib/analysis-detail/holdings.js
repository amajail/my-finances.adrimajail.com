...
export function renderDuplications(groups) {
  ...
  document.getElementById('dups-tbody').innerHTML = groups.map((g) => {
    const placements = (g.placements || [])
      .map((p) => `${escapeHtml(p.broker)}/${escapeHtml(p.assetType)} (${escapeHtml(String(p.quantity))})`)
      .join(', ');
    ...
    return `
      <tr class="border-t border-[var(--color-border)]">
        ...
        <td class="px-4 py-2 text-right num-mono">${escapeHtml(String(g.totalValueUsd))}</td>
      </tr>`;
  }).join('');
