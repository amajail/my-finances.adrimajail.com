...
- **FR-019**: System MUST persist each run's outcome as exactly one analysis record keyed by the target date, including: a one-paragraph executive summary (the same prose that the narrative body's "Executive Summary" section opens with — persisted as its own field so the dashboard list page can render it without parsing the full body), the full narrative body, the prompt version used, the model identifier used, the country-risk reading and its as-of date, run status (one of: completed, failed), and on failure, a short human-readable reason.
