...
- [ ] T024 Add a one-off bootstrap script at `scripts/seed-analysis-settings.js` that issues `PUT /api/settings/{key}` calls for the four analysis settings (`analysis.model` default `claude-opus-4-7`, `analysis.promptVersion` default `weekly-rebalance-v1`, `analysis.maxInputTokens` default `80000`, `analysis.maxOutputTokens` default `8000`). Add a section in `README.md` (or `quickstart.md`) on how to run it.
