...
- [x] T001 In `src/application/use-cases/analysis/GenerateWeeklyAnalysis.js`, after the snapshot is
...
- [x] T002 [P] [US1] Extend `tests/unit/domain/services/AllocationDriftCalculator.test.js`: a
...
- [x] T003 [P] [US1] Add a use-case-level test asserting the partition feeds only `investableSnapshot`
  to `AllocationDriftCalculator.computeDrift` / `computeConcentrationCaps` / `PositionChangeCalculator.diff`.

### Implementation for User Story 1

- [x] T004 [US1] In `GenerateWeeklyAnalysis.js`, pass `investableSnapshot` (not the full snapshot) to
...
- [x] T005 [US2] In `src/domain/entities/WeeklyAnalysis.js`, add optional `_administrativePositions`
...
- [x] T006 [US2] In `src/infrastructure/repositories/AzureAnalysisRepository.js`, write
  `administrativePositionsJson` when non-empty and parse it back (mirror `positionChangesJson`).
- [x] T007 [US2] In `GenerateWeeklyAnalysis.js`, pass `administrativePositions` to the
  `WeeklyAnalysis` constructor and the `captured` failure object. (FR-006, FR-007)
- [x] T008 [P] [US2] In `dashboard/src/pages/analysis-detail.astro`, add an "Administrative /
...
- [x] T009 [US2] Verify SC-004: a holding with `currentPrice: null` but positive `valueUsd`
...
- [x] T010 [US3] In `GenerateWeeklyAnalysis._buildUserMessage`, build `## currentHoldings` from the
...
- [x] T012 `npm test` green; `cd dashboard && npm run build` succeeds.
- [x] T013 Privacy self-review of the diff (tests/docs use placeholders only).
